...
cceye is local-first. It stores usage metadata and cost provenance locally, but does not store prompt or response bodies. Configuration, cache, state, and database files are created with user-only permissions where supported. API keys are read from environment variables and are not written to configuration or diagnostic output.
